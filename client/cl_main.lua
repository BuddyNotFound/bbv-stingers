...
function Main:PlaceSpike()
    ...
    CreateThread(function()
        ...
        while PlacingObject do
		    ...
		    if #(GetEntityCoords(CurrentObject) - GetEntityCoords(PlayerPedId())) >= Config.Settings.MaxDistance then Wrapper:Notify(Lang.SpikesCantBePlaced) self:CancelPlacement(CurrentObject) return end			
                Wait(100)
...
RegisterNetEvent('bbv-spikes:sync:client',function(data,id,pos)
    ...
    Wrapper:Target('Spike'..SpikeProp, 'Pick Up',  pos ,'removeprop:Spike'..SpikeProp .. tostring(randomnumber), 1.5 , 1.5)
    RegisterNetEvent('removeprop:Spike'..SpikeProp.. tostring(randomnumber),function()
